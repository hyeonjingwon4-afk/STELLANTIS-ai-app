                    ...
                    <table class="data-table" style="font-size:0.8rem;">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><label class="custom-checkbox-card" style="margin:0;padding:0.25rem 0.5rem;"><input type="checkbox" id="joint-hole-ok" checked /> <span>홀막힘 없음</span></label></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
...
// Insert HTML right after prep-spec-card ends (after line 598)
const prepCardEndMarker = '                <div class="clip-sub-card mt-3">\n                  <label class="custom-checkbox-card" style="margin-bottom:0;">\n                    <input type="checkbox" id="prep-chk-blade" />\n                    <span style="font-weight:700;"><i class="ri-tools-line"></i> 칼날교환주기 확인 완료</span>\n                  </label>\n                </div>\n              </div>\n            </div>';

if (html.includes(prepCardEndMarker)) {
  html = html.replace(prepCardEndMarker, prepCardEndMarker + '\n' + jointCardHTML);
} else {
  console.error("prepCardEndMarker not found!");
}

// 2. Process Visibility Logic in JS (isJoint)
const processVisibilityTarget = `    const isPrep = (proc?.name || '').includes('소재준비') || (proc?.id || '').includes('prep');
    container.querySelector('#prep-spec-card').style.display = isPrep ? 'block' : 'none';
    if (isPrep) this._initPrepCard(container, proc);`;

const processVisibilityReplacement = `    const isPrep = (proc?.name || '').includes('소재준비') || (proc?.id || '').includes('prep');
    container.querySelector('#prep-spec-card').style.display = isPrep ? 'block' : 'none';
    if (isPrep) this._initPrepCard(container, proc);
...
if (html.includes(processVisibilityTarget)) {
  html = html.replace(processVisibilityTarget, processVisibilityReplacement);
} else {
  console.error("processVisibilityTarget not found!");
}

// 3. JS Method: _initJointCard
const initJointCardJS = `
...
const initPrepTarget = `    container.querySelectorAll('.prep-LH-def').forEach(inp => inp.addEventListener('input', () => calcPrepDef('LH')));
    container.querySelectorAll('.prep-RH-def').forEach(inp => inp.addEventListener('input', () => calcPrepDef('RH')));
  }`;

if (html.includes(initPrepTarget)) {
  html = html.replace(initPrepTarget, initPrepTarget + '\n' + initJointCardJS);
} else {
  console.error("initPrepTarget not found!");
}

fs.writeFileSync(targetFile, html, 'utf8');
console.log("Successfully patched Joint Spec Card in index.html!");
