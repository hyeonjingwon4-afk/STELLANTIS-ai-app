...
with open('extracted_backup/index.html', 'r', encoding='utf-8') as f:
    html = f.read()
...
"""
html = html.replace('<div class="card mb-3">\n              <div class="card-header">\n                <div class="card-title"><i class="ri-file-edit-line"></i> 2. 생산 수량 &amp; 비가동 입력</div>', joint_card_html + '              <div class="card-header">\n                <div class="card-title"><i class="ri-file-edit-line"></i> 2. 생산 수량 &amp; 비가동 입력</div>')
...
# 4. _populateLogUI
pop_log_orig = """  _populateLogUI(container) {
...
pop_log_new = """  _populateLogUI(container) {
    const log = this.currentLog;
    const pwBadge = container.querySelector('#prework-status-badge');
    const pwBtn = container.querySelector('#btn-prework-submit');
...
# 5. _saveLog
save_log_orig = """  _saveLog(container, status) {
...
save_log_new = """  _saveLog(container, status) {
    if (!this.currentLog) return;
    if (!this.currentLog.preWorkDone) { alert('작업 시작 전 점검 일지를 먼저 제출해 주세요!'); return; }
    if (status === 'SUBMITTED' && !confirm('최종 제출 후에는 수정이 불가능합니다. 제출하시겠습니까?')) return;
...
save_log_orig_2 = """    Object.assign(this.currentLog, {
...
save_log_new_2 = """    let jointData = null;
    if (isJoint) {
      jointData = {
