...
# Define the good HTML content
good_html = """            <div id="joint-spec-card" class="card mb-3" style="display:none;">
...
# Replace by regex
pattern = re.compile(r'<div id="joint-spec-card".*?<!-- 6. .*?</div>\s*</div>\s*</div>\s*</div>', re.DOTALL)
match = pattern.search(content)

if match:
    new_content = content[:match.start()] + good_html + content[match.end():]
