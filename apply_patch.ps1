...
for ($i = 0; $i -lt $lines.Length; $i++) {
    ...
    # 1. Insert Joint Card HTML after prep-chk-blade
    if ($line -match 'id="prep-chk-blade"') {
        $newLines.Add($line)
        for ($j = 1; $j -le 4; $j++) {
            $i++
            $newLines.Add($lines[$i])
        }
        $newLines.Add($jointCardHTML)
        Write-Host "Joint Card HTML inserted"
        continue
    }

    # 2. Insert process visibility logic
    if ($line -match 'if \(isPrep\) this\._initPrepCard') {
        $newLines.Add($line)
        $newLines.Add('')
        $newLines.Add('    const isJoint = (proc?.name || "").includes("조인트") || (proc?.id || "").includes("joint");')
        $newLines.Add('    const jointCard = container.querySelector("#joint-spec-card");')
        $newLines.Add('    if (jointCard) jointCard.style.display = isJoint ? "block" : "none";')
        $newLines.Add('    if (isJoint) this._initJointCard(container, proc);')
        Write-Host "Process visibility logic inserted"
        continue
    }

    # 3. Insert _initJointCard method
    if ($line -match 'calcPrepDef\(''RH''\)\);') {
        $newLines.Add($line)
        $newLines.Add('  }')
        $newLines.Add($methodCode)
        $i++ # skip original closing brace
        Write-Host "_initJointCard method inserted"
        continue
    }
...
Write-Host "apply_patch.ps1 finished successfully."
